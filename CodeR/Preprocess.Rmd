...
```{r most_recent_entries_panels}
  ...
  rbind(dx_incidence_table, dx_incidence_table %>% reframe(across(!sex_binary, ~sum(.x)), sex_binary = "*")) %>%                                
    flextable() %>% set_caption("Adults: Prevalence and Incidence of Dx by Sex") %>% autofit()

  # remove subjects without endpoint
  df_analysis <- df_analysis %>%
    filter(!is.na(migraine_incidence), !is.na(stroke_incidence))

  # document flow
  print("Participants (Valid data elements)")
  print(sprintf("All:       %6d", n_distinct(df_all$pid)))
  print(sprintf("Adults:    %6d", n_distinct(df_adults$pid)))
  print(sprintf("Recent:    %6d", n_distinct(df_recent$pid)))
  print(sprintf("Endpoints: %6d", nrow(df_analysis)))
  ...
  # document flow
  print(sprintf("Analysis:  %6d", nrow(df_analysis)))
  ...
  print(sprintf("Incidence over study years (excluding prevalent Migraine or prevalent Stroke), N=%d: Migraine (%d), Stroke (%d)",
                nrow(df_analysis),
                sum(df_analysis$migraine_incidence == 1),
                sum(df_analysis$stroke_incidence   == 1)))
...
```{r save}
  # save tidy tables in compressed format, document version of RStudio, R, and attacheds packages

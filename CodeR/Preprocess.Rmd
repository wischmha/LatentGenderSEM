...
```{r most_recent_entries_panels}
  ...
  for (var in names(df_table1 %>% select(where(is.factor)))) {
    ...
    df_table1 <- df_table1 %>%
      mutate(across(contains(var), ~ifelse(is.na(.x) | .x == "NA", "NA", labels[as.integer(.x)]))) %>%
      mutate(across(contains(var), ~factor(.x, levels = labels)))
  }
...
```{r missingness_patterns}
  ...
  pngplot("./Results_Correlation_NA", 1.0,
    ...
    ggplot(aes(x = var1, y = var2, fill = Pearson)) +
      ...
      labs(x = "", y = "", title = "Correlation for Missing Data (NA)") +
      scale_fill_gradient2(low = "red", mid = "white", high = "blue", limits = c(-1, 1)) +
      theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 5)))
```
...
```{r corrplots_spearman}
  ...
  pngplot("./Results_Correlation_Spearman", 1.0,
    ...
    ggplot(aes(x = var1, y = var2, fill = Spearman)) +
      geom_tile() +
      scale_y_discrete(limits = rev) +
      labs(x = "", y = "", title = "Correlation for Data (Pairwise !NA)") +
      scale_fill_gradient2(low = "red", mid = "white", high = "blue", limits = c(-1, 1)) +
      theme(axis.text.y = element_text(size = 5), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 5)))
```

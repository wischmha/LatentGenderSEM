...
```{r sem_fit}
  ...
  # analyze unimputed data only, with weighting (important for representativeness, partially mitigate missing data)
  sem_unimp  <- sem(sem_model, data = df_con %>% filter(.imp == 0), auto.fix.first = FALSE, sampling.weights = "weight_factor",
                                                                                            sampling.weights.normalization = "none",
                                                                                            observed.information = "h1")

  # extract results from fit objects
  ...
  tab_unimp  <- summary(sem_unimp,  ci = TRUE, fit.measures = TRUE)
...
```{r sem_outcomes}
  ...
  tab_mi_age$pe %>% filter(grepl("incidence", lhs), op == "~") %>%
    ...
    hline(i = 8) %>% autofit()
  tab_unimp$pe %>% filter(grepl("incidence", lhs), op == "~") %>%
    mutate(estimate = sprintf("%.3f [%.3f, %.3f]", est, ci.lower, ci.upper)) %>%
    mutate(sig = print_p(pvalue)) %>%
    select(incidence = lhs, predictor = rhs, estimate, sig) %>%
    mutate(incidence = gsub("_incidence", "", incidence)) %>%
    flextable() %>% set_caption("SEM Regressions (unimputed data, main model, weighted)") %>%
    hline(i = 9) %>% autofit()

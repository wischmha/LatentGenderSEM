...
```{r sem_fit}
  ...
  # analyze un-imputed data only
  sem_cc <- sem(sem_model, data = df_con %>% filter(.imp == 0))
  tab_cc <- summary(sem_cc, ci = TRUE, fit.measures = TRUE)
  tab_cc$pe %>% filter(grepl("incidence", lhs), op == "~") %>%
    ...
    flextable() %>% set_caption("SEM Regressions (unimputed data, main model)") %>%
    hline(i = 9) %>% autofit()
